...
def _clean_adduct_metadata(metadata) -> dict:
    ...
    Spectrum, SpectraCollection, or None
        Input object with cleaned ``adduct`` metadata, or ``None`` if the input
        was ``None``.
    """
    ...
    cleaned_adduct = _clean_adduct(adduct, charge)

    updates = {}

    if charge:
        if charge != get_charge_of_adduct(cleaned_adduct):
            logger.warning(
                "The charge in the adduct: %s and the given charge: %s do not match",
                adduct,
                charge,
            )
    else:
        ...
        if charge_from_adduct:
            logger.info(
                "Unknown charge was derived from adduct: %s, now charge is %s",
                cleaned_adduct,
                charge_from_adduct,
            )
            updates["charge"] = charge_from_adduct

    if adduct != cleaned_adduct:
        updates["adduct"] = cleaned_adduct
...
def _add_missing_brackets_to_adduct(adduct):
    """Add missing brackets to an adduct and move the charge outside."""
...
def _convert_int_charge_to_str(charge):
    """Convert integer charge to adduct charge string."""
    if charge is None:
        return None
...
def _convert_known_adduct(adduct):
    """Convert adduct if a conversion rule is known."""
...
clean_adduct = metadata_update_filter(_clean_adduct_metadata)