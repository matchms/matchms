...
def require_retention_time(spectrum_in: SpectrumType,
    ...
    if not isinstance(retention_time, (float, int)):
        logger.warning("The retention time: %s is not a float or int, consider running add_retention first",
                       str(retention_time))
        return None
    if minimum_rt:
        if retention_time < minimum_rt:
            logger.info("The retention time %s, was smaller than the minimum_rt %s and is therefore set to None",
                        str(retention_time), str(minimum_rt))
            return None
    if maximum_rt:
        if retention_time > maximum_rt:
            logger.info("The retention time %s, was larger than the minimum_rt %s and is therefore set to None",
                        str(retention_time), str(maximum_rt))
            return None
    return spectrum
