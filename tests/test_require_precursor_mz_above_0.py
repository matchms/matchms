...
def test_require_precursor_mz_above_0_pass():
    ...
    spectrum = require_precursor_mz_above_0(spectrum_in)

    assert spectrum == spectrum_in, "Expected no changes."


def test_require_precursor_mz_above_0_fail_because_zero():
...
def test_require_precursor_mz_above_0_fail_because_below_zero():
    """Test if spectrum is None when precursor_mz < 0"""
    mz = numpy.array([10, 20, 30, 40], dtype="float")
    intensities = numpy.array([0, 1, 10, 100], dtype="float")
    spectrum_in = Spectrum(mz=mz, intensities=intensities)
...
def test_require_precursor_mz_above_0_without_precursor_mz():
...
def test_require_precursor_mz_above_0_with_wrong_precursor_mz():
    ...
    with pytest.raises(AssertionError) as msg:
        _ = require_precursor_mz_above_0(spectrum_in)

    assert "Expected 'precursor_mz' to be a scalar number." in str(msg.value)


def test_require_precursor_mz_above_0_with_input_none():
