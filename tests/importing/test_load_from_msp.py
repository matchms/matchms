...
def assert_matching_mass(molecule, expected_mass):
    assert np.isclose(molecule.get("parent_mass"), expected_mass, rtol=1e-5), "Expected different InChIKey."
...
def test_load_from_msp_spaces_mona_1():
    ...
    spectra_file = os.path.join(module_root, "testdata", "MoNA-export-GC-MS-first10.msp")
...
def test_load_from_msp_spaces_massbank_1():
    ...
    module_root = os.path.join(os.path.dirname(__file__), "..")
    spectra_file = os.path.join(module_root, "testdata", "massbank_five_spectra.msp")
    spectrum = list(load_from_msp(spectra_file))
    ...
    expected_parent_comment = [428.31, 141.0193, 267.1856, 300.1473, 415.234]
...
def test_load_from_msp_tabs():
    ...
    module_root = os.path.join(os.path.dirname(__file__), "..")
    spectra_file = os.path.join(module_root, "testdata", "rcx_gc-ei_ms_20201028_perylene.msp")
    spectra = list(load_from_msp(spectra_file))

    expected_inchikey = np.array(["CSHWQDPOILHKBI-UHFFFAOYSA-N"])
    ...
    expected_peak_comments = {
        113.03854: "Theoretical m/z 113.039125, Mass diff 0 (0 ppm), Formula C9H5",
...
def test_load_from_msp_multiline():
    ...
    module_root = os.path.join(os.path.dirname(__file__), "..")
    spectra_file = os.path.join(module_root, "testdata", "multiline_semicolon.msp")

    actual = list(load_from_msp(spectra_file))
    expected = [
        Spectrum(
            mz=np.array([12, 24, 25, 26, 27, 35, 36, 37, 38, 39, 47, 48, 49, 50, 51]).astype(float),
            intensities=np.array([0, 0, 2, 4, 0, 19, 26, 120, 49, 5, 25, 11, 60, 104, 13]).astype(float),
            metadata={"name": "Compound A", "num peaks": "15"},
        ),
        Spectrum(
            mz=np.array([40, 41, 42, 43, 44, 46, 50, 51, 52, 53]).astype(float),
            intensities=np.array([147, 57, 13, 52, 30, 1, 1, 8, 6, 1]).astype(float),
            metadata={
                "name": "JWH 081",
                "num peaks": "10",
            },
...
def test_load_from_msp_diverse_spectrum_collection():
    ...
    expected_parent_mass = np.array([224.083729624, 125.0241797459999])
    for k, n in enumerate(spectrum):
        assert_matching_mass(n, expected_parent_mass[k])

    expected_adducts = np.array(["M-H", "[M-H]-", "M+H"])
    for k, n in enumerate(spectrum):
