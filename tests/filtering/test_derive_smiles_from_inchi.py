...
def test_derive_smiles_from_inchi():
    ...
    spectrum_in = (
        SpectrumBuilder().with_metadata({"inchi": '"InChI=1S/C6H12/c1-2-4-6-5-3-1/h1-6H2"', "smiles": ""}).build()
    )
...
def test_derive_smiles_from_defect_inchi():
    ...
    pytest.importorskip("rdkit")
    spectrum_in = (
        SpectrumBuilder().with_metadata({"inchi": '"InChI=1S/C6H12/c1-2-XA4-6-5-3-1/h1-6H2"', "smiles": ""}).build()
    )
