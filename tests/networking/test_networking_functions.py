...
def test_get_top_hits_by_references():
    ...
    for key, value in scores_ref.items():
        assert np.allclose(value, expected_scores_ref[key], atol=1e-5), "Expected different selected scores"
    for key, value in idx_ref.items():
        assert np.array_equal(value, expected_idx_ref[key]), "Expected different selected indices"

    # Test lower top_n
    idx_ref, scores_ref = get_top_hits(scores, top_n=2, search_by="references")
    for key, value in scores_ref.items():
        assert np.allclose(value, expected_scores_ref[key][:2], atol=1e-5), "Expected different selected scores"
    for key, value in idx_ref.items():
        assert np.array_equal(value, expected_idx_ref[key][:2]), "Expected different selected indices"
...
def test_get_top_hits_by_queries():
    ...
    expected_scores_query = {
        "query_spec_0": np.array([1.0, 0.66666667, 0.66666667, 0.5]),
        "query_spec_1": np.array([0.66666667, 0.66666667, 0.5, 0.5]),
        "query_spec_2": np.array([0.8, 0.8, 0.5, 0.5, 0.5]),
    }
    expected_idx_query = {
        "query_spec_0": np.array([4, 2, 0, 3], dtype=np.int64),
    ...
    for key, value in scores_query.items():
        assert np.allclose(value, expected_scores_query[key], atol=1e-5), "Expected different selected scores"
    for key, value in idx_query.items():
        assert np.array_equal(value, expected_idx_query[key]), "Expected different selected indices"

    # Test lower top_n
    idx_query, scores_query = get_top_hits(scores, top_n=2, search_by="queries")
    for key, value in scores_query.items():
        assert np.allclose(value, expected_scores_query[key][:2], atol=1e-5), "Expected different selected scores"
    for key, value in idx_query.items():
        assert np.array_equal(value, expected_idx_query[key][:2]), "Expected different selected indices"
