...
from matchms.similarity.FlashSimilarity import (
    FlashSimilarity,
...
def test_pair_fragment_basic(use_ppm, tol):
    ...
    fse = FlashSimilarity(tolerance=tol, use_ppm=use_ppm, matching_mode="fragment",
...
def test_pair_returns_zero_when_empty_after_cleanup():
    ...
    fse = FlashSimilarity(tolerance=0.02, matching_mode="fragment",
...
def test_identity_gate_da_and_ppm():
    ...
    gate_da_tight = FlashSimilarity(tolerance=0.02, matching_mode="fragment",
                                         identity_precursor_tolerance=0.2, identity_use_ppm=False,
                                         remove_precursor=False, noise_cutoff=0.0)
    assert float(gate_da_tight.pair(s1, s2)) == 0.0

    gate_da_loose = FlashSimilarity(tolerance=0.02, matching_mode="fragment",
    ...
    gate_ppm_tight = FlashSimilarity(tolerance=0.02, matching_mode="fragment",
                                          identity_precursor_tolerance=300.0, identity_use_ppm=True,
                                          remove_precursor=False, noise_cutoff=0.0)
    assert float(gate_ppm_tight.pair(s1, s2)) == 0.0

    gate_ppm_loose = FlashSimilarity(tolerance=0.02, matching_mode="fragment",
...
def test_dtype_output_and_commutativity():
    ...
    f32 = FlashSimilarity(dtype=np.float32, remove_precursor=False, noise_cutoff=0.0)
    f64 = FlashSimilarity(dtype=np.float64, remove_precursor=False, noise_cutoff=0.0)
...
def test_matrix_dense_matches_pair():
    ...
    fse = FlashSimilarity(tolerance=0.1, use_ppm=False, matching_mode="fragment",
                               remove_precursor=False, noise_cutoff=0.0,
