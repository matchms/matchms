...
def _make_test_spectra():
    ...
    spectrum_2 = Spectrum(
        mz=np.array([100.0, 155.0, 205.0], dtype="float"),
...
def test_modified_cosine_wrapper_score_schema():
    modified_cosine = ModifiedCosine()

    assert modified_cosine.is_commutative is True
    assert tuple(modified_cosine.score_fields) == ("score", "matches")

    dtype_names = np.dtype(modified_cosine.score_datatype).names
    assert dtype_names == ("score", "matches")


def test_modified_cosine_pair_matches_greedy_when_use_hungarian_false():
    spectrum_1, spectrum_2, _ = _make_test_spectra()

    wrapped = ModifiedCosine(tolerance=0.1, use_hungarian=False)
    direct = ModifiedCosineGreedy(tolerance=0.1)
...
def test_modified_cosine_pair_matches_hungarian_when_use_hungarian_true():
    spectrum_1, spectrum_2, _ = _make_test_spectra()
...
def test_modified_cosine_pair_and_matrix_are_consistent_for_single_pair():
    spectrum_1, spectrum_2, _ = _make_test_spectra()

    similarity = ModifiedCosine(tolerance=0.1, use_hungarian=True)
...
def test_modified_cosine_symmetric_matrix_has_expected_fields_and_shape(use_hungarian):
    ...
    scores = ModifiedCosine(use_hungarian=use_hungarian).matrix(spectra, **kwargs)
