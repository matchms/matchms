...
def test_modified_cosine_hungarian_with_mass_shift(peaks, tolerance, masses, expected_matches):
    ...
    assert matches == len(expected_matches), "Expected differnt number of matching peaks."
...
def test_modified_cosine_hungarian_reduces_to_cosine_hungarian_for_zero_shift():
    ...
    modified_score, mod_matches = modified_cosine_hungarian.pair_scores_and_nr_of_matches(spectrum_1, spectrum_2)
    cosine_score, cos_matches = cosine_hungarian.pair_scores_and_nr_of_matches(spectrum_1, spectrum_2)

    assert modified_score == pytest.approx(cosine_score, 1e-12)
    assert mod_matches == cos_matches
...
def test_modified_cosine_hungarian_outperforms_approximation_on_conflicting_matches():
    ...
    exact, matches = ModifiedCosineHungarian(tolerance=0.01).pair_scores_and_nr_of_matches(spectrum_1, spectrum_2)
    approx, approx_matches = ModifiedCosineGreedy(tolerance=0.01).pair_scores_and_nr_of_matches(spectrum_1, spectrum_2)

    assert exact == pytest.approx(0.994475, 0.0001)
    assert matches == 2
    assert approx_matches == 1
    assert exact > approx
